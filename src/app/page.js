...
function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white shadow-sm" : ""}`}>
      ...
      <div className={`max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-18 sm:h-20 ${scrolled ? "" : "bg-navy"}`}>
        <a href="#" className="shrink-0">
          <Image src="/logo.png" alt="Yara's Cleaning" width={1077} height={526} className={`h-12 sm:h-16 w-auto ${scrolled ? "" : "brightness-0 invert"}`} priority />
        </a>
...
function WhyYaras() {
  ...
  return (
    <section id="why-yaras" className="py-20 sm:py-28 bg-navy">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <R>
          <div className="text-center max-w-2xl mx-auto mb-14">
            <h2 className="text-3xl sm:text-4xl font-bold text-white">Why families choose Yara&apos;s</h2>
            ...
            {points.map((p) => (
              <div key={p.title} className="bg-white/10 rounded-xl p-6 border border-white/10">
                <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center mb-4">
                  <p.Icon className="w-5 h-5 text-white/80" />
                </div>
